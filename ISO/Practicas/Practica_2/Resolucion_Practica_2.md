...
*(Los enunciados b, c, d y e correspondientes al lote de 5 procesos).*
...
- **SJF** arroja los tiempos promedios más bajos matemáticamente (TPR 7.8, TPE 3.6), ya que adelanta rápidamente los procesos livianos (J3 y J5) penalizando al más pesado (J2).
- **RR (q=6)** se comporta de forma **directamente idéntica a FCFS** para este lote, ya que el tamaño del quantum (6) es mayor o igual a la duración de cualquier ráfaga pedida, por ende nadie es desalojado por el reloj.
- **RR (q=1)**, por el contrario, reporta los peores promedios métricos de espera y retorno por retrasar las finalizaciones para dar la sensación de concurrencia e incluir un enorme overhead operativo.

### d. Conclusión sobre el Quantum de Round Robin
A grandes rasgos, **el tamaño del quantum dicta la esencia de RR**:
- Si el *quantum* es inmenso y holgado, RR se transforma estructuralmente en **FCFS**.
- Si el *quantum* es diminuto (q=1), se optimiza el tiempo de reacción (interactividad) pero disminuye drásticamente el rendimiento crudo por culpa del *overhead* constante del CPU cambiando contextos.

### e. ¿Cuándo usar Quantum Alto? (Ventajas y Desventajas)
- **Cuándo usarlo:** Se utiliza en sistemas orientados a procesamiento masivo pesado o de tipo *Batch*, donde la prioridad es terminar la tarea completa a tope de rendimiento de hardware, y no importa tanto dejar a los demás en el banco de suplentes esperando.
- **✔️ Ventajas:** Reduce prácticamente a cero el coste oculto o impacto de sobrecarga (*overhead*) por cambio del contexto; mejorando drásticamente el uso limpio de la CPU.
- **❌ Desventajas:** Pésimo nivel de respuesta. Arruina toda capacidad interactiva de cara al usuario e inyecta la típica falla de "Efecto Convoy" de FCFS.
